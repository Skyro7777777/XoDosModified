...
import com.xodos.shared.xodos.xodosConstants.xodos_APP.RUN_COMMAND_SERVICE;
 ...
 * plugins that contains info on command execution and forwards the extras to {@link xodosService}
 * for the actual execution.
 *
 * Check https://github.com/xodos/xodos-emulator/wiki/RUN_COMMAND-Intent for more info.
 */
public class RunCommandService extends Service {
        ...
        */
        ...
        if (replaceCommaAlternativeCharsInArguments) {
            ...
            if (commaAlternativeCharsInArguments == null)
                commaAlternativeCharsInArguments = xodosConstants.COMMA_ALTERNATIVE;
            // Replace any commaAlternativeCharsInArguments characters with normal commas
            DataUtils.replaceSubStringsInStringArrayItems(executionCommand.arguments, commaAlternativeCharsInArguments, xodosConstants.COMMA_NORMAL);
        }
        ...
        // user knows someone tried to run a command in xodos context, since it may be malicious
        ...
        errmsg = xodosPluginUtils.checkIfAllowExternalAppsPolicyIsViolated(this, LOG_TAG);
        if (errmsg != null) {
            executionCommand.setStateFailed(Errno.ERRNO_FAILED.getCode(), errmsg);
            xodosPluginUtils.processPluginExecutionCommandError(this, LOG_TAG, executionCommand, true);
            return stopService();
        }
        ...
        executionCommand.executable = xodosFileUtils.getCanonicalPath(executionCommand.executable, null, true);
        ...
        if (executionCommand.workingDirectory != null && !executionCommand.workingDirectory.isEmpty()) {
            ...
            executionCommand.workingDirectory = xodosFileUtils.getCanonicalPath(executionCommand.workingDirectory, null, true);
            ...
            // under allowed xodos working directory paths.
            // We try to set execute permissions, but ignore if they are missing, since only read and write permissions are required
            // for working directories.
            error = xodosFileUtils.validateDirectoryFileExistenceAndPermissions("working", executionCommand.workingDirectory,
                true, true, true,
                false, true);
            if (error != null) {
                executionCommand.setStateFailed(error);
        ...
        executableExtra = xodosFileUtils.getExpandedxodosPath(executableExtra);
        ...
        Intent execIntent = new Intent(xodos_SERVICE.ACTION_SERVICE_EXECUTE, executionCommand.executableUri);
        execIntent.setClass(this, xodosService.class);
        execIntent.putExtra(xodos_SERVICE.EXTRA_ARGUMENTS, executionCommand.arguments);
        execIntent.putExtra(xodos_SERVICE.EXTRA_STDIN, executionCommand.stdin);
        if (executionCommand.workingDirectory != null && !executionCommand.workingDirectory.isEmpty()) execIntent.putExtra(xodos_SERVICE.EXTRA_WORKDIR, executionCommand.workingDirectory);
        execIntent.putExtra(xodos_SERVICE.EXTRA_RUNNER, executionCommand.runner);
        execIntent.putExtra(xodos_SERVICE.EXTRA_BACKGROUND_CUSTOM_LOG_LEVEL, DataUtils.getStringFromInteger(executionCommand.backgroundCustomLogLevel, null));
        execIntent.putExtra(xodos_SERVICE.EXTRA_SESSION_ACTION, executionCommand.sessionAction);
        execIntent.putExtra(xodos_SERVICE.EXTRA_SHELL_NAME, executionCommand.shellName);
        execIntent.putExtra(xodos_SERVICE.EXTRA_SHELL_CREATE_MODE, executionCommand.shellCreateMode);
        execIntent.putExtra(xodos_SERVICE.EXTRA_COMMAND_LABEL, executionCommand.commandLabel);
        execIntent.putExtra(xodos_SERVICE.EXTRA_COMMAND_DESCRIPTION, executionCommand.commandDescription);
        execIntent.putExtra(xodos_SERVICE.EXTRA_COMMAND_HELP, executionCommand.commandHelp);
        execIntent.putExtra(xodos_SERVICE.EXTRA_PLUGIN_API_HELP, executionCommand.pluginAPIHelp);
        execIntent.putExtra(xodos_SERVICE.EXTRA_PENDING_INTENT, executionCommand.resultConfig.resultPendingIntent);
        execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_DIRECTORY, executionCommand.resultConfig.resultDirectoryPath);
        if (executionCommand.resultConfig.resultDirectoryPath != null) {
            execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_SINGLE_FILE, executionCommand.resultConfig.resultSingleFile);
            execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_FILE_BASENAME, executionCommand.resultConfig.resultFileBasename);
            execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_FILE_OUTPUT_FORMAT, executionCommand.resultConfig.resultFileOutputFormat);
            execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_FILE_ERROR_FORMAT, executionCommand.resultConfig.resultFileErrorFormat);
            execIntent.putExtra(xodos_SERVICE.EXTRA_RESULT_FILES_SUFFIX, executionCommand.resultConfig.resultFilesSuffix);
        }

        // Start xodos_SERVICE and pass it execution intent
    ...
    private void runStartForeground() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            startForeground(xodosConstants.xodos_RUN_COMMAND_NOTIFICATION_ID, buildNotification());
        }
    }
    ...
    private Notification buildNotification() {
        ...
        Notification.Builder builder =  NotificationUtils.geNotificationBuilder(this,
            xodosConstants.xodos_RUN_COMMAND_NOTIFICATION_CHANNEL_ID, Notification.PRIORITY_LOW,
            xodosConstants.xodos_RUN_COMMAND_NOTIFICATION_CHANNEL_NAME, null, null,
    ...
    private void setupNotificationChannel() {
        ...
        NotificationUtils.setupNotificationChannel(this, xodosConstants.xodos_RUN_COMMAND_NOTIFICATION_CHANNEL_ID,
            xodosConstants.xodos_RUN_COMMAND_NOTIFICATION_CHANNEL_NAME, NotificationManager.IMPORTANCE_LOW);
    }
